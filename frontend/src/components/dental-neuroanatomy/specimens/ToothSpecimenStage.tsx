...
// ============================================================================
// 2. ULTRA-DETAILED ORGANIC ENDODONTIC CROSS-SECTION SPECIMEN
// ============================================================================
const OrganicToothCrossSectionMesh: React.FC<{
  fdi: number;
  ...
  return (
    <group position={[0, 0, 0]}>
      ...
      {showPdl && !isPulpOnly && (
        <group position={[0, -0.024, 0]}>
          ...
          <mesh position={[0, -0.010, 0]}>
            <cylinderGeometry
              args={[crownRadiusNeck + 0.008, crownRadiusNeck + 0.012, 0.048, 32, 1, true, 0, isCut ? Math.PI : Math.PI * 2]}
            />
            <meshStandardMaterial
              color="#e6dbcb"
              roughness={0.8}
          ...
          <mesh position={[0, -0.008, 0]}>
            <cylinderGeometry
              args={[crownRadiusNeck + 0.002, crownRadiusNeck + 0.004, 0.044, 32, 1, true, 0, isCut ? Math.PI : Math.PI * 2]}
            />
            <meshStandardMaterial
              color="#06b6d4"
              roughness={0.4}
      ...
      {/* 2. MEN RĂNG THÂN RĂNG (Enamel Crown with Organic Contour) */}
      {!isPulpOnly && (
        <group position={[0, 0.020, 0]}>
          {/* Anatomically curved crown */}
          ...
          {isMolar && (
            <group position={[0, 0.017, 0]}>
              ...
              <mesh position={[-0.012, 0.002, 0.012]}>
                <sphereGeometry args={[0.009, 16, 16, 0, isCut ? Math.PI : Math.PI * 2, 0, Math.PI / 2]} />
              ...
              <mesh position={[-0.012, 0.001, -0.012]}>
                <sphereGeometry args={[0.008, 16, 16, 0, isCut ? Math.PI : Math.PI * 2, 0, Math.PI / 2]} />
                <meshStandardMaterial color="#fffef7" roughness={0.25} />
              </mesh>
              {/* DL Cusp */}
              <mesh position={[0.012, 0.001, -0.012]}>
      ...
      {!isPulpOnly && (
        <group position={[0, -0.010, 0]}>
          ...
          {/* Ngà chân răng (Root Dentin) */}
          <group position={[0, -0.012, 0]}>
            {isMolar ? (
              <>
                {/* Chân Gần (Mesial Root) */}
                <mesh position={[-0.009, -0.010, 0]} rotation={[0, 0, 0.06]}>
                  <cylinderGeometry
                    args={[0.008, 0.003, 0.038, 16, 1, false, 0, isCut ? Math.PI : Math.PI * 2]}
                  />
                ...
                <mesh position={[0.009, -0.010, 0]} rotation={[0, 0, -0.06]}>
                  <cylinderGeometry
                    args={[0.007, 0.003, 0.036, 16, 1, false, 0, isCut ? Math.PI : Math.PI * 2]}
                  />
                  <meshStandardMaterial color="#e2ce9c" roughness={0.5} side={THREE.DoubleSide} />
                </mesh>
              </>
              ...
              <mesh position={[0, -0.010, 0]}>
                <cylinderGeometry
                  args={[crownRadiusNeck - 0.003, 0.0025, 0.042, 16, 1, false, 0, isCut ? Math.PI : Math.PI * 2]}
                />
                <meshStandardMaterial color="#e2ce9c" roughness={0.5} side={THREE.DoubleSide} />
              </mesh>
            )}
      ...
      {/* 4. BUỒNG TỦY & HỆ THỐNG ỐNG TỦY (Pulp Chamber & Canals) */}
      <group position={[0, 0, 0]}>
        {/* Buồng tủy thân răng (Pulp Chamber) */}
        <mesh position={[0, 0.012, 0]}>
          <boxGeometry args={[isMolar ? 0.016 : 0.008, 0.012, isMolar ? 0.014 : 0.007]} />
          <meshStandardMaterial
            color="#e11d48"
            emissive="#e11d48"
        ...
        {isMolar && (
          <group position={[0, 0.018, 0]}>
            <mesh position={[-0.006, 0, 0.005]}>
              <coneGeometry args={[0.002, 0.005, 8]} />
              <meshStandardMaterial color="#e11d48" emissive="#e11d48" emissiveIntensity={1.0} />
            </mesh>
            <mesh position={[0.006, 0, 0.005]}>
              <coneGeometry args={[0.002, 0.005, 8]} />
              <meshStandardMaterial color="#e11d48" emissive="#e11d48" emissiveIntensity={1.0} />
        ...
        <group position={[0, -0.022, 0]}>
          {isMolar ? (
            <>
              {/* Ống gần ngoài (MB) */}
              <mesh position={[-0.009, 0, 0.004]} rotation={[0, 0, 0.05]}>
...
export const ToothSpecimenStage: React.FC = () => {
  ...
  // Toggle between Real 3D Dental Arch View and Organic Cross-Section Specimen
  ...
  const cameraPosition = useMemo(() => {
    if (viewMode === 'arch') {
      ...
      return [x * 1.5 + 0.05, y + 0.02, z + 0.09] as [number, number, number];
    }
    return [0.12, 0.04, 0.15] as [number, number, number];
  }, [viewMode, currentToothInnervation]);
  ...
  return (
    <div className="relative w-full h-full select-none overflow-hidden bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950">
      ...
      <div className="absolute top-4 right-4 z-20 flex flex-col gap-2 pointer-events-auto max-w-xs">
        ...
        {/* Cross-Section Parameters (Only visible in Isolated mode) */}
        {viewMode === 'isolated' && (
          <div
            className={`p-3 rounded-2xl border backdrop-blur-md shadow-xl ${
              isDark ? 'bg-slate-900/90 border-slate-800' : 'bg-[#f7f2ea]/90 border-[#dfd5c6]'
      ...
      >
        ...
        <directionalLight position={[-0.5, 0.2, -0.5]} intensity={0.9} />
        <pointLight position={[cameraTarget[0], cameraTarget[1] + 0.05, cameraTarget[2] + 0.05]} intensity={1.5} color="#fffef5" />

        {/* View Mode 1: Full 3D Dental Arch & In-Situ Teeth */}
        {viewMode === 'arch' ? (
